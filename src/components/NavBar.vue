  ...
  <nav class="sticky top-0 z-50 backdrop-blur-md bg-slate-900/95 border-b border-slate-800 shadow-md transition-all duration-300">
    <div class="max-w-7xl mx-auto px-4 md:px-8">
      <div class="flex justify-between items-center h-16">
        ...
        <div class="hidden md:flex items-center gap-1.5 font-semibold text-sm">
          ...
          <!-- NEW: Calendar / Schedule link -->
          <router-link to="/calendar" active-class="bg-indigo-600 text-white border border-indigo-500" class="text-slate-300 hover:text-white hover:bg-slate-800 px-3.5 py-2 rounded-xl transition-all flex items-center gap-2">
            📅 Schedule
          </router-link>
    ...
    <Transition name="slide">
      <div v-if="mobileOpen" class="md:hidden bg-slate-900/98 border-t border-slate-800 px-4 py-4 flex flex-col gap-2 font-semibold text-sm shadow-inner max-h-[calc(100vh-4rem)] overflow-y-auto">
        ...
        <router-link to="/calendar" @click="mobileOpen = false" active-class="bg-indigo-600 text-white" class="text-slate-300 hover:text-white hover:bg-slate-800 p-3 rounded-xl transition-all flex items-center gap-3">
          <i class="fas fa-calendar-alt w-5 opacity-70 text-center"></i> Doctor's Schedule
        </router-link>
